...
class AffiliateTable(tables.Table):
    ...
    def render_subs(self, value):
        return mark_safe(f'<a href="{reverse("sub_affiliate_view", args=[value])}" class="btn btn-secondary">عرض</a>')
    
    # def render_subaffiliates(self, value):
    #     # Create a link to the sub_affiliate_view
    #     return mark_safe(f'<a href="{reverse("sub_affiliate_view", args=[value])}" class="btn btn-link">عرض الفروع</a>')
